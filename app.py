...
def main():
    ...
    def load_data():
        data = pd.read_csv("/Users/sandeepnutakki/Documents/Python Projects/ML Streamlit Learning Project/mushrooms.csv")
        labelencoder=LabelEncoder()
        for col in data.columns:
            data[col] = labelencoder.fit_transform(data[col])
        return data
    
    #Creating train and test datasets by spliting in 70,30 ratios
    @st.cache(persist=True)
    def split(df):
        y = df.type
    ...
    #using the load function and standardizing calumn with real names
    df = load_data()
    class_names = ['edible', 'poisonous']
    
    x_train, x_test, y_train, y_test = split(df)

    #Adding Algorthim selection field to the website
    ...
    #Show raw data by check box selection
